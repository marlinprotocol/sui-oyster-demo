# SUI Price Oracle - Oyster Enclave (Node.js)

Node.js HTTP server that fetches SUI token price from CoinGecko and signs it with secp256k1 for on-chain verification. Deployed as an Oyster Enclave using Oyster CVM CLI.
...
```bash
cd enclave_node
docker build -t <your-dockerhub-username>/sui-price-oracle-node .
```

### 2. Push to Docker Hub

```bash
docker push <your-dockerhub-username>/sui-price-oracle-node:latest
```
...
docker build -t <username>/sui-price-oracle-node .
docker push <username>/sui-price-oracle-node:latest
