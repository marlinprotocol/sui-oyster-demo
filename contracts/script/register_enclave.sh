...
#
# Usage: ./register_enclave.sh <registry_package_id> <registry_id> <enclave_ip> <attestation_port>

if [ "$#" -lt 3 ] || [ "$#" -gt 4 ]; then
    echo "Usage: $0 <registry_package_id> <registry_id> <enclave_ip> [attestation_port]"
...
sui client ptb --assign v "vector$ATTESTATION_ARRAY" \
    ...
    --assign result \
    --move-call "${REGISTRY_PACKAGE_ID}::enclave_registry::register_enclave" @${REGISTRY_ID} result \
