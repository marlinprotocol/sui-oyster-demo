# SUI Price Oracle - Oyster Enclave (Python)

Python HTTP server that fetches SUI token price from CoinGecko and signs it with secp256k1 for on-chain verification. Deployed as an Oyster Enclave using Oyster CVM CLI.
...
```bash
cd enclave_python
docker build -t <your-dockerhub-username>/sui-price-oracle-python .
```

### 2. Push to Docker Hub

```bash
docker push <your-dockerhub-username>/sui-price-oracle-python:latest
```
...
docker build -t <username>/sui-price-oracle-python .
docker push <username>/sui-price-oracle-python:latest
